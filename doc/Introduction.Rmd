...
Given $N$ sets of series data: $\boldsymbol{y} = \{y_i(t_j): j\in[n_i]\}_{i=1}^{N}$, where $n_i$ is the length of the $i$-th series, we assume that each series $y_i$ represents $n_i$ measurements over the continuous treatment $t$ at levels $\{t_j\}_{j=1}^{n_i}$.
