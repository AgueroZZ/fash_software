...
The goal of \code{fashr} is to perform functional adaptive shrinkage (FASH) for inferring the posterior distribution of the effect size function $\beta_i(t)$, given the observed data $\boldsymbol{\hat{\beta}}_i$ and $\boldsymbol{s}_i$.
...
\]
where each GP component $\text{GP}_k$ is specified via the differential equation: if $\beta(t) \sim \text{GP}_k$, then
\[
...
FASH estimates the prior mixing weights $\hat{\boldsymbol{\pi}}$ by maximizing the marginal likelihood of the observed effect size estimates:
\[
...
Based on the estimated prior $\hat{g}$, FASH then computes the posterior distribution for $\beta_i(t)$ as:
\[
